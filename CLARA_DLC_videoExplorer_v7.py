...
class LabelsPanel(wx.Panel):
    ...
    def addButtons(self, guiDim):
        ...
        self.omit_label = wx.ToggleButton(self, size=(50, -1), id=wx.ID_ANY, label="Omit")
        self.labelBox.Add(self.omit_label, 0, wx.ALL|wx.ALIGN_CENTER, buttSpace)
        self.omit_label.Enable(False)
        
        self.grab_labels = wx.ToggleButton(self, size=(50, -1), id=wx.ID_ANY, label="Grab")
...
# Spliting the frame into top and bottom panels. Bottom panels contains the widgets. The top panel is for showing images and plotting!
        ...
        self.image_panel = ImagePanel(vSplitterA,self.gui_size, 3)
...
# Add Buttons to the WidgetPanel and bind them to their respective functions.
        ...
        self.labelselect = self.choice_panel.addRadioButtons(['none'],self.guiDim)
...
#        Making radio selection for vid speed
    ...
    def movePts(self, mag):
        if len(self.labelselect) > 1:
            cat = self.labelselect[0].GetSelection()+1
            bp = self.labelselect[cat].GetString(self.labelselect[cat].GetSelection())
        else:
        ...
        if dlgP.ShowModal() == wx.ID_OK:
            ...
            if self.use_syn.GetValue():
                ...
                if not len(mounted_serv):
                    ...
                    return
                syntag = '/gvfs/smb-share:server=synology,share=whsynology/BIOElectricsLab/RAW_DATA/AutomatedBehavior'
                startDir = PurePath(mounted_serv[0] + syntag)
            ...
            if dlgD.ShowModal() == wx.ID_OK:
                working_directory = os.path.join(dlgD.GetPath(),experimenter)
                dlgD.Destroy()
...
# =============================================================================
        ...
        if len(mounted_serv):
            syntag = '/gvfs/smb-share:server=synology,share=whsynology/BIOElectricsLab/RAW_DATA/AutomatedBehavior'
            synDir = str(PurePath(mounted_serv[0] + syntag))
...
#            wildcard = "Avi files (*.avi)|*.avi"
        ...
        self.labelselect = self.choice_panel.addRadioButtons(self.bodyparts,self.guiDim)
        ...
        efile = 'none'
        if len(mounted_serv):
            sessStr = '_'.join(os.path.split(self.videoList[0])[1].split('_')[0:3])
            annotag = '/gvfs/smb-share:server=synology,share=whsynology/BIOElectricsLab/ANALYZED_DATA/AutomatedBehavior/ReachingEvents/'
            efile = sessStr+'_reachAnno.txt'
            efile = str(PurePath(mounted_serv[0] + annotag + efile))
...
#            frame = frame[cpt[2]:cpt[2]+cpt[3],cpt[0]:cpt[0]+cpt[1]]
            ...
            for bpndx, bp in enumerate(self.bodyparts):
                ...
                circle = [patches.Circle((self.points[0], self.points[1]), radius=self.markerSize,
                                         linewidth=4, fc = color, alpha=0.0)]
...
#        self.axes[vndx].set_ylim([ylim[1],ylim[0]])
        ...
        self.frmrate = self.user_cfg['framerate']
    ...
    def showStats(self, event):
        ...
        for f in f_list:
            ...
            for d in data_list:
                ...
                datastats = statData.count().get_values()
                for n,s in enumerate(range(0,len(datastats),2)):
                    runTot[n]+=datastats[s]
    ...
    def chooseFrame(self, event):
        for ndx, hax in enumerate(self.axes):
            ...
            if hand_test or class_test:
                self.labelselect.SetSelection(len(self.bodyparts)+bpindexH)
                if class_test:
                    event.xdata = int(self.df_x[ndx][bpindexC,self.currFrame])
                    event.ydata = int(self.df_y[ndx][bpindexC,self.currFrame])
                    event.xdata = event.xdata+cpt[0]
                    event.ydata = event.ydata+cpt[2]
                else:
                    event.xdata = int(self.df_x[ndx][bpindexH,self.currFrame])
                    event.ydata = int(self.df_y[ndx][bpindexH,self.currFrame])
                    event.xdata = event.xdata+cpt[0]
                    event.ydata = event.ydata+cpt[2]
                self.onClick(event)
            if class_test:
                self.labelselect.SetSelection(bpindexC)
                event.xdata = int(self.df_x[ndx][bpindexC,self.currFrame])
                event.ydata = int(self.df_y[ndx][bpindexC,self.currFrame])
                event.xdata = event.xdata+cpt[0]
                event.ydata = event.ydata+cpt[2]
                self.onClick(event)
            if pellet_test:
                self.labelselect.SetSelection(len(self.bodyparts)+bpindexP)
                event.xdata = int(self.df_x[ndx][bpindexP,self.currFrame])
                event.ydata = int(self.df_y[ndx][bpindexP,self.currFrame])
                event.xdata = event.xdata+cpt[0]
                event.ydata = event.ydata+cpt[2]
                self.onClick(event)
        self.move_label.SetValue(0)
        ...
        if self.label_frames.GetValue():
            ...
            for ndx, hax in enumerate(self.axes):
                if event.inaxes == hax:
                    ...
                    self.y1 = event.ydata
                    cpt = self.cropPts[ndx]
                    self.x1 = self.x1-cpt[0]
                    self.y1 = self.y1-cpt[2]
    ...
    def loadAnnotations(self, event):
        if self.show_tracking.GetValue():
            ...
            for vndx, video in enumerate(self.videos):
                ...
                self.textH.append(self.axes[vndx].text(10,190,' ',color=[1,1,1],fontsize=16))
